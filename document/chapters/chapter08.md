...
# 쿠버네티스와 컨테이너 오케스트레이션
- 컨테이너의 발전으로 인해 여러 머신에서 컨테이너를 관리하는 솔루션을 찾기 시작했다.
- 도커에서도 도커 스웜 등 컨테이너 오케스트레이션 솔루션을 제공했지만 쿠버네티스가 가장 많이 사용되고 있다.

## 컨테이너 오케스트레이션에 대한 사례
- 쿠버네티스는 컨테이너 오케스트레이션 또는 컨테이너 스케줄러로 불리기도 한다.
- 우리는 좀 더 안정적인 운영을 위해 중복된 컨테이너를 띄워 여러 머신에 분산시키길 원한다.
- 이런 의미에서 컨테이너를 스케줄링하여 가용한 자원을 찾고 필요한 경우 재할당하여 운영을 하기 위한 세세한 작업을 쿠버네티스가 대신해준다.
- 여러 머신의 기대 상태를 관리하여 우리가 의도한 대로 운영할 수 있도록 도와준다.
- 워크로드를 분산하는 방법을 지정할 수 있기 때문에 리소스 사용률, 프로세스간 레이턴시 등을 최적화할 수 있다.
- 다양한 플랫폼에도 이와같은 기능을 제공해주지만 쿠버네티스가 가장 많이 사용되고 있다.

## 쿠버네티스의 개념
![img.png](../images/kubernetes.png)
- 쿠버네티스 클러스터는 두 부분으로 구분 되는데 워크로드가 실행되는 노드와 클러스터를 관리하는 컨트롤 플레인으로 구성된다.
- 컨트롤 플레인은 컨테이너를 스케줄링하는 대신 파드라는 단위로 관리한다. (파드는 하나 이상의 컨테이너로 구성되어 있는 단위이다.)
- 일반적으로 파드에서는 하나의 컨테이너만 실행하지만 때로는 서비스 매시의 개념으로 여러 컨테이너를 실행할 수 있다.
![img.png](../images/service.png)
- 쿠버네티스에서 서비스는 내부에서 실행되는 파드들이 안정적인 네트워크 인터페이스를 제공하는 역활을 한다.
- 파드는 여러 이유로 종료될 수 있기 때문에 종료가 되더라도 나머지 파드들로 라우팅하여 새로운 파드가 실행되도록 할 수 있다.
- 쿠버네티스는 서비스가 아닌 파드를 배포하기 때문에 시간이 좀 더 걸릴 수 있다.
- 레플리카셋: 파드의 기대 상태를 유지하기 위해 사용되는 장치이다.
- 디플로이먼트: 파드와 레플리카셋의 상태를 관리하는 장치이다. 최신버전노드, 롤백, 노드 수 확장등의 작업을 수행할 수 있다.

## 멀티테넌시와 페더레이션
- 쿠버네티스는 가용한 자원을 모아두고 조직 전체의 워크로드의 클러스터를 관리할 수 있고 필요한 상황에 따라 할당하기 때문에 자원의 활용도가 높아진다.
- 쿠버네티스는 서로 다른 마이크로서비스를 관리하는데 좋은 방법이지만 여러 팀, 프로젝트, 사용자에 따라 자원을 격리시키는 것은 어렵다.
- ![img.png](../images/federation.png)
- 해결 방법
  - 레드햇의 오픈 시프트와 같이 멀티테넌시를 지원하는 플랫폼을 사용하는 방법
  - 페더레이션 모델을 사용하는 방법, 여러 클러스터위에 관리하는 계층을 두어 각 클러스터를 관리하는 방법
  - 단점은 서로 다른 클러스터에 노드를 이동하는 것이 어렵다는 것이다.
  - 하나의 노드는 하나의 클러스터나 다른 클러스터의 일부일 수 있기 때문에 A와 B 양쪽에 파드를 실행해두는 것이 좋다.
  - 새로운 파드로 업데이트할 때 하나의 클러스터 내부에서 업데이트 되는 것 보다 이미 업그레이드된 클러스터로 마이크로서비스를 이동시키는 것이 더 쉽고 안전하다.
- 결국 규모의 문제고 더 큰 규모의 효율성을 원할 때 고려해보는 것이 좋다.

> 쿠버네티스의 배경  
> 전서계 데이터센터에 있는 컨테이너를 관리하기 위헤 구글에서 만든 오메가라는 오픈소스에서 차용해 만들어진 것이 쿠버네티스이다.
> 쿠버네티스는 개발자 친화적인 환경을 제공하길 원했고 이 오픈소스를 만드는 데 많은 엔지니어링을 투자했다.
> 이러한 시도들을 통해 AWS라는 클라우드 서비스의 독점을 깨고자 했으며 이러한 노력으로 쿠버네티스는 성공적인 오픈소스 프로젝트가 되었다.

## 클라우드 네이티브 컴퓨팅 재단
- CNCF는 직접 개발하고 생성하기 보다는 동일한 곳에 호스팅하여 공통적으로 표준을 정의하고 상호 운용성을 확보하는 것을 목표로 한다.
- 문서 및 교욱 자료를 제공하고 쿠버네티스에 대한 다양한 인증 프로그램을 정의한다.

## 플랫폼의 이식성
- 쿠버네티스은 플랫폼이 아니다. 자체적인 플랫폼을 구축할 수 있도록 도와주는 도구이다.
- 이러한 플랫폼을 만드는 것은 쉽지 않다. 쿠버네티스는 호환성이 많은 도구로 인해 선택을 해야될 때 어려움이 있다.
- 일부 플랫폼 서비스는 그러한 선택을 제한하기도 한다. 
- 현재 플랫폼을 쓰다가 다른 플랫폼으로 이동한다고 하면 다시 빌드해야될 수도 있다.
- 하나의 클러스터에서 작동하는 마이크로서비스를 다른 클러스터로 이동하는 것은 이론적으로 가능하지만 실제로 항상 가능하진 않다.

## 헬름, 오퍼레이터, CRD
- 오퍼레이터: 초기 설치에 필요한 패키지들을 설치하고 관리하는 역활을 한다.
- 헬름: 오퍼레이터에서 지원하지 않는 패키지들을 설치하고 관리하는 역활을 한다.
- CRD: 쿠버네티스의 API를 확장하는 방법이다. 쿠버네티스의 API를 확장하여 새로운 리소스를 정의하고 이를 사용할 수 있게 해준다.

## Knative
- Knative는 서버리스 환경을 쿠버네티스 위에서 구축할 수 있도록 도와주는 프로젝트이다.
- 서비스 메시(Istio)는 Knative에서 제공하는 서비스로 서버리스 환경을 구축할 수 있도록 필수로 사용해야한다.
- Faas와 같은 제품을 만들 때 Knative를 사용하는 것은 아직 이르다.
- CNCF의 프로젝트에 기부하지 않기로 했다.(지금은 기부한 상태임)

## 미래
- 사설 클라우드를 위해 자체 쿠버네티스 클러스터를 구축하거나 공용 클라우드에서 관리형 클러스터를 사용하게 될 것이다.
- 아직까지는 개발자 친화적이다고 할 수 없지만 더 나아가서 Knative와 같이 더 높은 수준의 추상황화를 하게 된다면 개발자 친화적인 환경을 제공할 것이다.

## 사용해야할까?
- 쿠버네티스를 관리하는 것은 상당히 어렵고 복잡한 작업이다.
- 전문 회사에 아웃소싱하는 방법
- 클라우드 서비스를 사용하는 방법
- 쿠버네티스를 남들이 하고 있어서 사용하는 것은 좋지 않은 생각이다. 마이크로서비스가 많지 않는다면 쿠버네티스를 사용할 필요가 없다.

# 점진적 제공
- 성과가 좋은 회사는 더 잦은 배포를 하고 변경 실패율도 훨씬 낮다라는 연구 결과가 있다.
- 그래서 더 자주 출시하고 실패율을 낮춤으로써 릴리즈 방식도 바뀌었다.

## 배포와 릴리스의 분리
> 배포는 소프트웨어가 설치되는 것이고 릴리스는 사용자가 사용할 수 있는 상태로 만드는 것이다.
- 블루그린 배포가 이러한 개념을 활용한 예중 하나이다.
- 블루그린 배포: 새로운 버전의 서비스를 배포하고 트래픽을 새로운 서비스로 전환하는 방법

## 점진적 제공으로
- 폭팔반경(영향받는 범위)를 최소화하여 지속적으로 제공하는 방법
- 고객에 가장 가까이 있는 프로덕트 오너가 점진적인 제공을 할 수 있도록 도와주는 것이 중요하다.
- 프로덕트 오너는 점진적 제공에 대해 매커니즘을 이해하고 있어야하기 때문에 기술적으로 어느정도 이해할 수 있어야한다.

## 기능 토글
- 특정 기능을 끄거나 켤 수 있다. 토글 뒤에 배포된 기능을 숨길 수 있다.
- 좀 더 세분화된 토글 방법으로는 그룹에 따라 토글을 적용할 수 있다.

## 카나리아 배포
- 컴퓨터는 우리가 실수를 했을 때 더 빠르고 큰 규모로 실수를 할 수 있게 해준다.
- 카나리아 릴리즈는 이러한 실수로 인한 영향력을 최소화하기 위한 방법이다.
- 카나리아 릴리즈는 제한된 일부 고객들만 새로운 버전을 사용하게 하고 문제가 발생하면 다른 고객들에게 영향을 미치지 않도록 하는 방법이다.
- 마이크로서비스 수준에서 토글을 구현하여 해당 기능의 사용 여부를 결정할 수 있다.
- 서로 다른 버전의 마이크로서비스를 나란히 실행하고 토글을 통해 라우팅을 하는 전략도 있다.

## 병렬 실행
- 동일한 기능의 서로 다른 구현을 나란히 실행하고 두 구현체에 동일한 요청을 보내어 결과를 비교하는 방법이다.
- 하나의 구현체는 신뢰할 수 있다고 가정하고 보통 기존 구현체가 이에 해당한다.
- 사이언티스트와 같은 툴은 이러한 패턴을 사용하는데 도와준다.
